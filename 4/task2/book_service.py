...
class BookRepository:
    ...
    def find_by_id(self, book_id: int) -> Book:
        # фильтр выступает в роли итератора и нам доступна функция next, поиск по id из списка
        return [*filter(lambda x: x.id == book_id, self._books)][0]

    def find_all(self) -> list[Book]:
        # Метод deepcopy рекурсивно создает копию каждого элемента объекта и не копирует ссылки.
