# Вспомогательный класс, имеющий ABCMetaметакласс. С помощью этого класса можно создать абстрактный базовый класс, просто унаследовав его от ABC
...
class Vehicle(metaclass=ABCMeta):
    ...
    # Декоратор @abstractmethod может использоваться для объявления абстрактных методов свойств
    @abstractmethod
...
class Car(Vehicle):
    def __init__(self, company: str, model: str, year: int):
        # super().__init__() вызывает метод инициализации из родительского класса. Например, чтобы дополнить его.
